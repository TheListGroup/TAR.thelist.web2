...
def check_quota_credit(
    ...
    try:
        ...
        if rows['Member_Status'] == 'Normal':
            ...
            if quota == 0:
                return {'Status': False, 'Message': "ตอนนี้ credit การทำ questionnaire ของคุณหมดแล้ว กรุณาทำการยืนยันตัวตนเพื่อเป็น premium member โดยการยื่นเอกสารเพิ่มเติม หรือติดต่อเจ้าหน้าที่โดยตรง 094-653-3971, LINE: @REALLEASE , email: contact@real-lease.agency"}
            else:
                return {'Status': True}
        else:
            quota = rows['Premium_Search_Remaining']
            if quota == 0:
                return {'Status': False, 'Message': "ตอนนี้ credit การทำ questionnaire ของคุณหมดแล้ว กรุณาติดต่อเจ้าหน้าที่โดยตรง 094-653-3971, LINE: @REALLEASE , email: contact@real-lease.agency เพื่อขอ credit เพิ่มเติม"}
            else:
                return {'Status': True}
...
def admin_office_point(
    ...
    best_distance_yarn: str = Form(None), #var
    worst_distance_yarn: str = Form(None), #var
    best_distance_road: str = Form(None), #var
    worst_distance_road: str = Form(None), #var
    ...
    try:
        ...
        best_distance_yarn = 0.0 if not best_distance_yarn else float(best_distance_yarn) #var
        worst_distance_yarn = 500.0 if not worst_distance_yarn else float(worst_distance_yarn) #var
        best_distance_road = 100.0 if not best_distance_road else float(best_distance_road) #var
        worst_distance_road = 400.0 if not worst_distance_road else float(worst_distance_road) #var
        ...
        if location_yarn_list:
            location_yarn_regex = "|".join(location_yarn_list)
        else:
            location_yarn_regex = "xxx"
        location_road_list = [road.strip("'") for road in location_road.split(";")] if location_road else []
        if location_road_list:
            location_road_regex = "|".join(location_road_list)
        else:
            location_road_regex = "xxx"
        ...
        if location_station_list:
            location_station_regex = "|".join(location_station_list)
        else:
            location_station_regex = "xxx"
    ...
    try:        
        rows = _office_point_calculate(actime_start_default, actime_end_default, work_start, work_end, in_unit_condition, 
            ...
            dir_s_score, dir_e_score, dir_w_score, min_ceiling_clear, prefer_no_column, 
