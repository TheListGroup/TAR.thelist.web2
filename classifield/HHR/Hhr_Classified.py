...
save_folder, json_file = destination('HHR','hhr.json')
work,sheet = check_update(json_file,'https://docs.google.com/spreadsheets/d/10lKaZ5jPUqikTOxVFjEHoFSeS6nqaf_ZEWwjvkj2Jsc/edit#gid=0')
    ...
    if len(data_list) > 0:
        ...
        for data in data_list:
            ...
            if found:
                ...
                log, upd, stop_processing, row_sheet = update_work(stop_processing, cursor, connection, query, val, folder_url, classified_id, save_folder, log, upd, insert, create_date, last_updated_date, sheet, row_sheet, 'HHR')
                if stop_processing:
                    break
                ...
                log, insert, stop_processing, row_sheet = insert_work(stop_processing, cursor, connection, query, val, prop_id, user_id, folder_url, save_folder, log, upd, insert, create_date, last_updated_date, sheet, row_sheet, 'HHR')
                if stop_processing:
                    break
            row_sheet += 1
        
        if log:
            insert_log("HHR_insert_prop",log,upd,insert,cursor,connection,'')
