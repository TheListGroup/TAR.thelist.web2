...
#project_path = 'D:\PYTHON\TAR.thelist.web-1\classifield\AG\AG_PROJECT.json'
...
for i, proj in enumerate(project_list):
    ...
    create_date = proj[11]
    update_date = proj[12]
    use_project_list.append((ref_id,proj_id,name_th,name_en,lat,long,create_date,update_date))
...
if more_work:
    ...
    #save log
        query = """INSERT INTO realist_log (Type, SQL_State, Message, Location)
                VALUES (%s, %s, %s, %s)"""
        val = (0, '00000', f'Insert {insert} Rows', 'AG_Insert_Match_Project')
        try:
            cursor.execute(query,val)
            connection.commit()
        except Exception as e:
            print(f'Error: {e} at AG_Insert_log')
