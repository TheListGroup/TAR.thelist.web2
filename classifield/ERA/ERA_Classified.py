...
SHEET_URL = 'https://docs.google.com/spreadsheets/d/1cw22S-MOecJVTfLDt8svBEzlJ6jZ46UaW_4FVfxEW8c/export?format=csv'
...
save_folder, json_file = destination('ERA','era.json')
work,sheet = check_update(json_file,'https://docs.google.com/spreadsheets/d/1cw22S-MOecJVTfLDt8svBEzlJ6jZ46UaW_4FVfxEW8c/edit#gid=0')
...
if sql:
    ...
    if len(data_list) > 0:
        ...
        for data in data_list:
            ...
            stop_processing, column_check, log = check(stop_processing, query ,prop_id, user_id, cursor, log, upd, insert, connection, 'ERA')
            if stop_processing:
                break
            ...
            if found:
                ...
                log, upd, stop_processing, row_sheet = update_work(stop_processing, cursor, connection, query, val, folder_url, classified_id, save_folder, log, upd, insert, create_date, last_updated_date, sheet, row_sheet, 'ERA')
                if stop_processing:
                    break
                ...
                log, insert, stop_processing, row_sheet = insert_work(stop_processing, cursor, connection, query, val, prop_id, user_id, folder_url, save_folder, log, upd, insert, create_date, last_updated_date, sheet, row_sheet, 'ERA')
                if stop_processing:
                    break
            row_sheet += 1
        
        if log:
            insert_log("ERA_insert_prop",log,upd,insert,cursor,connection,'')
