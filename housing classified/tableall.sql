...
CREATE TABLE IF NOT EXISTS `housing_classified` (
    ...
    `Price_Sale` INT UNSIGNED NULL DEFAULT 0,
    ...
    `Price_Rent` INT UNSIGNED NULL DEFAULT 0,
...
-- -----------------------------------------------------
-- Table `housing_classified_log`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `housing_classified_log` (
    ...
    `Project_ID` VARCHAR(100) NULL,
    `Classified_Type` ENUM('เช่า', 'ขาย', 'เช่าและขาย') NULL,
    `Housing_Type` ENUM('บ้านเดี่ยว','บ้านแฝด','ทาวน์โฮม,','โฮมออฟฟิศ','อาคารพาณิชย์') NULL,
    `Housing_Code` VARCHAR(50) NOT NULL,
    `Housing_Latitude` DOUBLE NULL,
    `Housing_Longitude` DOUBLE NULL,
    `Price_Sale` INT UNSIGNED NULL DEFAULT 0,
    ...
    `Last_Update_Insert_Date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`ID`),
    INDEX hc_user (User_ID),
    INDEX hc_housing (Housing_Code))
ENGINE = InnoDB;