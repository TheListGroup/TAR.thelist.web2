...
save_file = r"C:\Users\RealResearcher1\Documents\GitHub\TAR.thelist.web2\propertyhub\classified_all.csv"
        ...
        if 'listings' in browser.current_url:
            try:
                ...
                for x, each_data in enumerate(classified_data.find_all('li')):
                    ...
                    if section == 'ราคา':
                        try:
                            classified_price_sale = int(re.findall(r'\d+', re.sub(',', '', each_data.find('span', {'class': 'sale-price'}).text))[0])
                            sale = 1
                        except:
        ...
        if classified_section:
            if page < 1 and not resume:
                ...
                for classified in all_classified:
                    ...
                    try:
                        classified_price_sale = int(classified['price']['forSale']['price'])
                        sale = 1
                    except:
                        classified_price_sale = None
                    try:
