...
SELECT
    ...
    replace(
        replace(
            replace(
                replace(replace(`a`.`Condo_ENName`, '\n', ''), '-', ''),
                '(',
                ''
            ),
            ')',
    ...
    (
        (
            (
                (
                    ifnull(`a`.`No_of_Unit_Point`, 0) + ifnull(`a`.`Finish_Year_Point`, 0)
                ) + ifnull(`a`.`HighRise_Point`, 0)
            ) + ifnull(`a`.`ListCompany_Point`, 0)
        ) + ifnull(`a`.`DistanceFromStation_Point`, 0)
    ) AS `Total_Point`,
    `station`.`Condo_Around_Station` AS `Condo_Around_Station`,
    if(
        (
            (`type1`.`studio` is null)
            and (`type2`.`onebed` is null)
            and (`type3`.`twobed` is null)
            and (`type4`.`threebed` is null)
        ...
        concat(
            ...
            ifnull(`type2`.`onebed`, ''),
            ifnull(`type3`.`twobed`, ''),
            ifnull(`type4`.`threebed`, ''),
            ifnull(`type5`.`fourbed`, '')
        )
    ) AS `Condo_Bedroom_Type`,
    if(
        (
            (`size_stu`.`stus` is null)
            and (`size_1bed`.`1beds` is null)
            and (`size_2bed`.`2beds` is null)
            and (`size_3bed`.`3beds` is null)
            and (`size_4bed`.`4beds` is null)
        ),
        '[1BR:20-60][2BR_1B:20-60]',
        concat(
            ifnull(`size_stu`.`stus`, ''),
            ifnull(`size_1bed`.`1beds`, ''),
            ifnull(`size_2bed`.`2beds`, ''),
            ifnull(`size_3bed`.`3beds`, ''),
            ifnull(`size_4bed`.`4beds`, '')
        )
    ) AS `Condo_Room_Size`,
    concat(
        if((`c`.`PS001` = 'Y'), '[PS001]', ''),
        ...
        if((`c`.`CUS042` = 'Y'), '[CUS042]', '')
    ) AS `Spotlight_List`,
    if(
        (`d`.`Condo_Built_Finished` is not null),
(
            year(curdate()) - year(`d`.`Condo_Built_Finished`)
        ),
        if(
            (`d`.`Condo_Built_Start` is not null),
            if(
                (`a`.`Condo_HighRise` = 1),
(
                                            ...
                                            (
                                                (
                                                    (
                                                        (
                                                            (
                                                                `condo_price_calculate_view` `b`
                                                                join `real_condo` `a` on((`b`.`Condo_Code` = `a`.`Condo_Code`))
                                                            )
                                                            join `condo_spotlight_relationship_view` `c` on((`a`.`Condo_Code` = `c`.`Condo_Code`))
                                                        )
                                                        left join `real_condo_price` `d` on((`a`.`Condo_Code` = `d`.`Condo_Code`))
                                                    )
                                                    left join (
                                                        select
                                                            `condo_around_station`.`Condo_Code` AS `Condo_Code`,
                                                            group_concat(
                                                                '[',
                                                                `condo_around_station`.`Station_Code`,
                                                                ']' separator ''
                                                            ) AS `Condo_Around_Station`
                                                        from
                                                            `condo_around_station`
                                                        group by
                                                            `condo_around_station`.`Condo_Code`
                                                    ) `station` on((`a`.`Condo_Code` = `station`.`Condo_Code`))
                                                )
                                                left join (
                                                    select
                                                        `cpc`.`Condo_Code` AS `Condo_Code`,
(
                                                            select
                                                                cast(`cv`.`Distance` as decimal(25, 20))
                                                            from
                                                                `condo_around_station_view` `cv`
                                                            where
                                                                (
                                                                    (`cpc`.`Condo_Code` = `cv`.`Condo_Code`)
                                                                    and (`cv`.`Condo_Code`, `cv`.`Total_Point`) in (
                                                                        select
                                                                            `cv`.`Condo_Code`,
                                                                            min(`cv`.`Total_Point`)
                                                                        from
                                                    ...
                                                    from
                                                        `condo_price_calculate_view` `cpc`
                                                ) `sub_distance` on((`a`.`Condo_Code` = `sub_distance`.`Condo_Code`))
                                            )
                                            left join (
                                                select
                                                    `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                                    '[STU]' AS `studio`
                                                from
                                                    `full_template_unit_type`
                                                where
                                                    (
                                                        (
                                                            `full_template_unit_type`.`Unit_Type_Status` <> 2
                                                        )
                                                        and (`full_template_unit_type`.`Room_Type_ID` = 1)
                                                    )
                                                group by
                                                    `full_template_unit_type`.`Condo_Code`
                                            ) `type1` on((`a`.`Condo_Code` = `type1`.`Condo_Code`))
                                        )
                                        left join (
                                            select
                                                `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                            ...
                                            where
                                                (
                                                    ...
                                                    and (`full_template_unit_type`.`Room_Type_ID` = 2)
                                                )
                                            group by
                                                `full_template_unit_type`.`Condo_Code`
                                        ) `type2` on((`a`.`Condo_Code` = `type2`.`Condo_Code`))
                                    )
                                    left join (
                                        select
                                            `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                        ...
                                        where
                                            (
                                                ...
                                                and (`full_template_unit_type`.`Room_Type_ID` = 4)
                                            )
                                        group by
                                            `full_template_unit_type`.`Condo_Code`
                                    ) `type3` on((`a`.`Condo_Code` = `type3`.`Condo_Code`))
                                )
                                left join (
                                    select
                                        `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                    ...
                                    where
                                        (
                                            ...
                                            and (`full_template_unit_type`.`Room_Type_ID` = 5)
                                        )
                                    group by
                                        `full_template_unit_type`.`Condo_Code`
                                ) `type4` on((`a`.`Condo_Code` = `type4`.`Condo_Code`))
                            )
                            left join (
                                select
                                    `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                ...
                                where
                                    (
                                        ...
                                        and (`full_template_unit_type`.`Room_Type_ID` = 6)
                                    )
                                group by
                                    `full_template_unit_type`.`Condo_Code`
                            ) `type5` on((`a`.`Condo_Code` = `type5`.`Condo_Code`))
                        )
                        left join (
                            select
                                `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                                concat(
                                    '[STU:',
                                    `roundSize`(min(`full_template_unit_type`.`Size`)),
                                    '-',
                                    `roundSize`(max(`full_template_unit_type`.`Size`)),
                                    ']'
                                ) AS `stus`
                            from
                                `full_template_unit_type`
                            ...
                            order by
                                `full_template_unit_type`.`Condo_Code`
                        ) `size_stu` on((`a`.`Condo_Code` = `size_stu`.`Condo_Code`))
                    )
                    left join (
                        select
                            `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                        ...
                        order by
                            `full_template_unit_type`.`Condo_Code`
                    ) `size_1bed` on((`a`.`Condo_Code` = `size_1bed`.`Condo_Code`))
                )
                left join (
                    select
                        `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                    ...
                    order by
                        `full_template_unit_type`.`Condo_Code`
                ) `size_2bed` on((`a`.`Condo_Code` = `size_2bed`.`Condo_Code`))
            )
            left join (
                select
                    `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
                ...
                order by
                    `full_template_unit_type`.`Condo_Code`
            ) `size_3bed` on((`a`.`Condo_Code` = `size_3bed`.`Condo_Code`))
        )
        left join (
            select
                `full_template_unit_type`.`Condo_Code` AS `Condo_Code`,
            ...
            order by
                `full_template_unit_type`.`Condo_Code`
        ) `size_4bed` on((`a`.`Condo_Code` = `size_4bed`.`Condo_Code`))
    )
...
BEGIN
    ...
    DECLARE cur CURSOR FOR SELECT Condo_ID, Condo_Code, Distance, Condo_Price_Per_Square, Condo_Price_Per_Square_Date,
                                ...
                                Condo_Segment, Condo_Around_Line
