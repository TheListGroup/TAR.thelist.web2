...
output_csv = r"C:\PYTHON\TAR.thelist.web2\scrap\Match_Developer_EN.csv"
...
for x in range(home.index.size):
    dont_have_en = False
    if pd.notna(home.iloc[x].iloc[2]):
        developer_enname = home.iloc[x].iloc[2].lower()
    if pd.notna(home.iloc[x].iloc[2]):
        format_list = [" ",r"\|","co.,ltd.",r"\.","–","’","'","plc","public","company","limited",r"\(.*?\)",","," "]
    ...
    for y in range(realist.index.size):
        if pd.notna(realist.iloc[y].iloc[2]) and pd.notna(home.iloc[x].iloc[2]): 
            developer_realisten = realist.iloc[y].iloc[2].lower()
            for word_clean in format_list:
            ...
            if (ratio == 1):
                ...
                developer_realisten_full = realist.iloc[y].iloc[2]
                break
    ...
    if dont_have_en == False:
        data_dict = {"ID": home.iloc[x].iloc[0], "LINK": home.iloc[x].iloc[1], "Developer_Full_EN": home.iloc[x].iloc[2], "Developer_EN": developer_enname
                    , "Dev_EN": best_word, "Dev_ENFull": developer_realisten_full, "Dev_Code": best_dev_code, "Point": best_ratio}
    else:
        data_dict = {"ID": home.iloc[x].iloc[0], "LINK": home.iloc[x].iloc[1], "Developer_Full_EN": "", "Developer_EN": "", "Dev_EN": ""
